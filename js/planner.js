...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // used by the "Regenerate" button, customDishes, used by the picker's
  // "quick add" row, or manualOrder, used by within-meal drag reordering)
  // existed won't have these fields yet.
  ...
  if (!mpCurrentPlan.manualOrder) mpCurrentPlan.manualOrder = {};
...
// Once a meal has been manually arranged via within-slot drag reordering
// (see moveDishBeforeTarget()), plan.manualOrder[slotKey] is set and that
// meal's dish list order is fully user-controlled — applying the carb-first
// cosmetic sort on top would silently undo whatever position the user just
// dragged a dish to. Untouched meals (autogenerated or manually built via
// the picker, but never drag-reordered) keep the original carb-first default.
function orderDishesForDisplay(slotKey, dishes) {
  if (mpCurrentPlan.manualOrder && mpCurrentPlan.manualOrder[slotKey]) return dishes;
  return sortDishesCarbFirst(dishes);
}
...
function buildSlotCell(slotDef, { showLabel = true } = {}) {
  ...
  if (dishes.length === 0) {
    ...
    orderDishesForDisplay(slotDef.key, dishes).forEach(dish => card.appendChild(buildSlotDishRow(slotDef.key, dish)));
  }
...
function buildSlotDishRow(slotKey, dish) {
  ...
  // Each dish row is also its own drop target — separate from the
  // card-level one in buildSlotCell(), which only handles cross-slot moves
  // that land on empty card space (always appends to the end). Dropping
  // directly on a row inserts before/after it (moveDishBeforeTarget()),
  // which is what makes same-meal reordering possible — dropping a dish
  // onto another dish in the same slot. stopPropagation() on drop (not
  // dragover, so the card's own hover highlight still shows) keeps the
  // card-level handler from also firing for the same drop.
  wrapper.addEventListener("dragover", e => {
    e.preventDefault();
  });
  wrapper.addEventListener("dragenter", () => {
    wrapper.classList.add("mp-slot-dish-drop-target");
  });
  wrapper.addEventListener("dragleave", () => {
    wrapper.classList.remove("mp-slot-dish-drop-target");
  });
  wrapper.addEventListener("drop", e => {
    ...
    if (!payload || !payload.slotKey || !payload.dishId) return;
    const rect = wrapper.getBoundingClientRect();
    const insertBefore = (e.clientY - rect.top) < rect.height / 2;
    moveDishBeforeTarget(payload.slotKey, slotKey, payload.dishId, dish.id, insertBefore);
  });
...
function clearSlotArray(slotKey) {
  ...
  if (mpCurrentPlan.manualOrder) delete mpCurrentPlan.manualOrder[slotKey];
  pruneUnusedCustomDishes();
  savePlan(mpCurrentPlan);
  renderPlannerGrid();
}

// Drag-and-drop between slots (e.g. Tue lunch -> Wed lunch), dropped onto
// the card itself rather than onto a specific dish row — always appends to
// the end, and (unlike moveDishBeforeTarget() below) leaves the destination
// slot's manualOrder alone, matching its pre-existing "just append, let the
// cosmetic carb-first sort place it" behavior. Carries the dish's per-dish
// note along to its new slot rather than dropping it — unlike
// removeDishFromSlot(), which deletes notes outright because there the dish
// is actually leaving the plan, not just moving within it.
...
// Row-level drop target (see buildSlotDishRow's per-row dragover/drop
// handlers) — dropping directly onto a dish inserts the dragged dish
// immediately before/after it, rather than always appending to the end like
// moveDishToSlot() above. This is what backs both same-meal reordering
// (fromSlotKey === toSlotKey) and a cross-slot move that lands on a specific
// dish rather than empty card space. Always marks the destination slot's
// manualOrder, since choosing an exact position only means something if the
// cosmetic carb-first sort (see orderDishesForDisplay()) stops overriding it.
function moveDishBeforeTarget(fromSlotKey, toSlotKey, dishId, targetDishId, insertBefore) {
  if (dishId === targetDishId) return;
  const fromIds = mpCurrentPlan.slots[fromSlotKey] || [];
  if (!fromIds.includes(dishId)) return;

  const sameSlot = fromSlotKey === toSlotKey;
  mpCurrentPlan.slots[fromSlotKey] = fromIds.filter(id => id !== dishId);

  const toIds = sameSlot ? mpCurrentPlan.slots[fromSlotKey] : (mpCurrentPlan.slots[toSlotKey] || []).slice();
  const targetIndex = toIds.indexOf(targetDishId);
  const insertIndex = targetIndex === -1 ? toIds.length : (insertBefore ? targetIndex : targetIndex + 1);
  toIds.splice(insertIndex, 0, dishId);
  mpCurrentPlan.slots[toSlotKey] = toIds;
  ...
  if (!mpCurrentPlan.manualOrder) mpCurrentPlan.manualOrder = {};
  mpCurrentPlan.manualOrder[toSlotKey] = true;
...
// Plain divs prefixed with a bullet character, not a real <ul>/<li> —
// mirrors the print-media block's own reasoning below (a real <ul> gets a
// hanging indent from its marker regardless of padding/margin overrides,
// especially once Word's own list-conversion engine gets involved when this
// HTML is opened as a .doc), so this is the only way to reliably get a
// zero-indent bullet line in the exported document.
function dishNamesHtml(slotKey) {
  ...
  return orderDishesForDisplay(slotKey, dishes)
    .map(dish => {
      const note = getDishNote(slotKey, dish.id);
      return `<div style="margin:0;padding:0;">&bull; ${escapeHtml(dish.name)}${note ? ` <em>(${escapeHtml(note)})</em>` : ""}</div>`;
    })
    .join("");
}
...
function buildPlanExportHtml() {
  ...
  // pt, not px — Word's HTML-to-.doc conversion renders px at 96dpi (so the
  // old "16px" actually showed as 12pt in Word's own font-size box, which is
  // what looked small); pt is Word's native unit, so this is exactly 13pt.
  let body = `<table style="border-collapse:collapse;width:100%;table-layout:fixed;font-family:Calibri,Arial,sans-serif;font-size:13pt;">`;
  if (mpCurrentPlan.scope === "week") {
    const dayW = "width:14%;";
